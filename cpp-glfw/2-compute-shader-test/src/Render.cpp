...
RenderShader::RenderShader(const std::string& vert_path, const std::string& frag_path):
...
i32 RenderShader::get_uniform_location(const std::string& name) {
    i32 res;
...
void RenderShader::set_uniform(const std::string& name, int x) {
    i32 location = get_uniform_location(name);
    GLCall(glUniform1i(location, x));
}

void RenderShader::set_uniform(const std::string& name, float x) {
    i32 location = get_uniform_location(name);
    GLCall(glUniform1f(location, x));
}

void RenderShader::set_uniform(const std::string& name, float x, float y) {
    i32 location = get_uniform_location(name);
    GLCall(glUniform2f(location, x, y));
}

void RenderShader::set_uniform(const std::string& name, float x, float y, float z) {
    i32 location = get_uniform_location(name);
    GLCall(glUniform3f(location, x, y, z));
}

void RenderShader::set_uniform(const std::string& name, float x, float y, float z, float w) {
    i32 location = get_uniform_location(name);
...
Texture::Texture(int width, int height, char* data) : width(width), height(height) {
    buffer = data ? data : new char[width * height * 4];
    ...
    GLCall(glBindImageTexture(1, renderer_id, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F)); 
}

Texture::Texture(int width, int height, float* data) : width(width), height(height) {
    buffer = data ? data : new float[width * height * 4];
    GLCall(glGenTextures(1, &renderer_id));
    GLCall(glBindTexture(GL_TEXTURE_2D, renderer_id));

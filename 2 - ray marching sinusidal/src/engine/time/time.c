...
void time_init(u32 frame_rate) {
	...
	global.time.start = (f32)SDL_GetTicks();
}

void time_update(void) {
	global.time.now = (f32)SDL_GetTicks() - global.time.start;
...
void time_delay(void) {
	global.time.frame_time = (f32)SDL_GetTicks() - global.time.start - global.time.now;

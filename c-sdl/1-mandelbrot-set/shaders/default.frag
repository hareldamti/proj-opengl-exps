...
void main() {
    ...
    float zoom_ = 1;
    vec2 uv = (2. * gl_FragCoord.xy/(vec2(1,1) * resolution.x) - vec2(1, 1)) / zoom_ - mouse_displacement;
    float intensity = 0;
    int anti_aliasing = 10;
    for (int i = 0; i < anti_aliasing; i++) intensity += fractal(uv + rand2(uv) / 2.0 / resolution.x / zoom_, exp(vec2(1, 0 * time_now/1e3))) / float(anti_aliasing);

...
float min_distance(vec3 position) {
    ...
    r = length(position);
    theta = acos(position.z / (r + threshold/10));
    phi = atan2(position.y, position.x);
    theta += time_now / 1e4 * 4 * s;
...
vec3 march_ray(vec3 camera_pos, mat3 camera_dir, vec2 xy) {
    ...
    while (d > threshold && bounds) {
        ...
        d = min_distance(camera_dir * (current_position - camera_pos) + camera_pos);
        if (d > d_last) glow += pow(d_last * 3000,-2);
    ...
    float intensity = (bounds ? surface : 0);

    return mix(colormap(intensity, CMAP_ORANGE), colormap(intensity, CMAP_PURPLE), 0.5 + 0.5 * (sin(5 * current_position.z)));
}
